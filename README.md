...
- 已实现：完整首页、一级页面、项目/活动/资源/画廊二级页、公开人员名录与成员详情、四个中心详情、考核结果登录续接、登录、招新报名、成员空间、帮助中心
...
| `/resources/[slug]` | 资源详情、文件状态与版本说明 | 公开浏览；内部下载按需登录 |
| `/assessment-results` | 考核结果占位页 | 登录后访问 |
| `/join` | 招新说明 | 公开 |
...
- 画廊专题通过 `/gallery/[slug]` 公开浏览。灯箱支持 `ArrowLeft`、`ArrowRight`、`Escape`，打开时聚焦关闭按钮并锁定背景滚动，关闭后焦点回到原照片。
- 考核结果属于个人数据；当前页面只说明数据尚未接入，不展示或虚构分数、评级、排名、评语、周期和通过状态。
