...
describe("admin content production mutations", () => {
  it("generates all reviewed content detail and workflow operations", async () => {
    expect(API_V1_PATHS).toMatchObject({ adminContentDetail: "/api/v1/admin/content/{contentId}", adminContentCreate: "/api/v1/admin/content" });
    ...
    expect(requests[1]?.body).toEqual({ expectedVersion: 3, reason: "Needs revision" });
    expect(requests[3]?.body).toEqual({ expectedVersion: 5, confirmed: true });
    expect(requests[4]?.body).toEqual({ expectedVersion: 6, confirmed: true });
    expect(requests[5]?.body).toEqual({ expectedVersion: 7, reason: "Expired" });
  });
  ...
  it("uses credentials, request IDs, and CSRF for content mutations", async () => {
    const response = { id: "content-1", publicId: "public-1", centerId: null, slug: "content-1", kind: "article", status: "draft", version: 1, createdBy: { type: "account", accountId: "owner", username: "owner", displayName: "Owner" }, createdAt: "2026-08-24T00:00:00.000Z", updatedAt: "2026-08-24T00:00:00.000Z", workingRevision: { revisionNumber: 1, title: "API title", summary: "API summary", tag: null, internalTarget: null, expiresAt: null, blocks: [{ type: "paragraph", text: "API body" }], internalNote: null }, publishedRevisionNumber: null, rejectionReason: null, publishedAt: null, offlineAt: null, offlineReason: null };
    ...
    expect(fetcher.mock.calls[0]?.[1]).toEqual(expect.objectContaining({ method: "PATCH", credentials: "include", headers: { "X-Request-ID": "content-request-id", "Content-Type": "application/json", "X-CSRF-Token": "csrf-token" } }));
  });
