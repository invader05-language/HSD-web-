...
- [ ] Write failing unit tests for time boundaries, manual override precedence, single-open enforcement, owner-only open/pause/close/reopen, application uniqueness, snapshots, and center removal.
- [ ] Run the focused Vitest files and confirm failures are caused by missing domain/store behavior.
- [ ] Implement the smallest domain types, fixtures, selectors, and commands required by the tests.
- [ ] Run focused tests again, then refactor only while green.
- [ ] Commit as `feat: add recruitment batch domain boundary`.
...
- [ ] Add failing tests for open, upcoming-only, and no-batch `/join` states, direct `/join/apply` blocking, login continuation, edit/withdraw/resubmit, closed-during-edit handling, and latest-batch-only results.
- [ ] Run the focused tests and confirm red status.
- [ ] Update the pages to show the batch name/time/centers from the store, disable or replace CTAs when unavailable, retain drafts without creating records, and display clear state reasons.
- [ ] Add batch labels to submitted summaries and remove hard-coded `2026 秋季招新`/`CURRENT` text.
- [ ] Run focused unit and E2E tests, then commit as `feat: link member recruitment to current batch`.
...
- [ ] Add failing tests for batch-context URLs, owner vs ordinary-admin permissions, confirmation-required early open, single-open blocking, and batch-isolated candidate/publication/export data.
- [ ] Run the focused E2E/unit tests and confirm red status.
- [ ] Implement batch list/detail context, lifecycle command controls, audit display, and batch-aware links/CSV naming while preserving existing admin shell behavior.
- [ ] Run focused tests and commit as `feat: add batch-scoped recruitment administration`.
...
- [ ] Search for remaining hard-coded batch labels, `applicationsByMemberId`, and page-local time comparisons.
...
- [ ] Run `NUXT_TELEMETRY_DISABLED=1 sh scripts/with-hsd-node.sh corepack pnpm run test:e2e`.
- [ ] Review the diff, preserve unrelated untracked tooling paths, and commit only if additional verification fixes are needed.
