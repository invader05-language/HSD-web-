...
<template>
  <div class="admin-recruitment-page admin-section-page">
    <section class="admin-forbidden-state">
      ...
      <dl><div><dt>当前账号</dt><dd>{{ account?.name ?? "未登录" }}（{{ account?.account ?? "-" }}）</dd></div><div><dt>当前管理级别</dt><dd>{{ getAdminLevelLabel(session.adminLevel) }}</dd></div><div><dt>缺少资格</dt><dd>{{ requiredAccess }}</dd></div></dl>
