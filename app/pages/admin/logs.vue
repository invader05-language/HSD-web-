...
import { presentAuditAction, presentAuditTarget, presentAuditSnapshot } from "~/services/audit/admin-audit-presenter";
...
<template>
  ...
  <div v-else class="admin-recruitment-page admin-section-page">
    ...
    <section class="admin-list-card">
      ...
      <div v-else-if="listStatus === 'success'" class="admin-table-scroll"><table aria-label="管理员操作日志"><thead><tr><th>操作人</th><th>动作</th><th>目标</th><th>时间</th><th><span class="sr-only">操作</span></th></tr></thead><tbody><tr v-for="record in rows" :key="record.id"><td><strong>{{ record.actor }}</strong><small>{{ record.actorAccount || record.actorType }}</small></td><td>{{ presentAuditAction(record.action) }}</td><td>{{ presentAuditTarget(record.targetType, record.targetId) }}</td><td>{{ record.occurredAt }}</td><td><button type="button" @click="realList?.select(record)">变更前 / 变更后</button></td></tr></tbody></table></div>
    ...
    <Teleport to="body"><div v-if="selected" class="admin-drawer-backdrop" @click.self="realList?.clearSelection()"><aside class="admin-candidate-drawer" aria-label="日志详情"><header class="admin-drawer__header"><div><span>AUDIT DETAIL</span><h2>{{ presentAuditAction(selected.action) }}</h2><p>{{ selected.occurredAt }} · {{ selected.actor }}</p></div><button type="button" aria-label="关闭日志详情" @click="realList?.clearSelection()">×</button></header><div class="admin-drawer__body"><section><header><span>01</span><h3>变更前 / 变更后</h3></header><div class="admin-audit-diff"><article><span>BEFORE</span><p>{{ presentAuditSnapshot(selected.before) }}</p></article><article><span>AFTER</span><p>{{ presentAuditSnapshot(selected.after) }}</p></article></div></section><section><header><span>02</span><h3>服务端安全字段</h3></header><dl class="admin-detail-grid"><div><dt>目标</dt><dd>{{ presentAuditTarget(selected.targetType, selected.targetId) }}</dd></div><div v-if="selected.reason"><dt>原因</dt><dd>{{ selected.reason }}</dd></div></dl></section></div><footer class="admin-drawer__footer"><button type="button" class="button" @click="realList?.clearSelection()">关闭</button></footer></aside></div></Teleport>
  </div>
</template>
