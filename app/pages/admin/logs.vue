...
import { createAdminAuditListController } from "~/services/audit/admin-audit-list";
import { formatAuditOccurredAt, formatAuditProjection, presentAuditAction, presentAuditTarget } from "~/services/audit/admin-audit-presenter";
...
<template>
  ...
  <div v-else class="admin-recruitment-page admin-section-page">
    ...
    <section class="admin-list-card">
      ...
      <div v-else-if="listStatus === 'success'" class="admin-table-scroll"><table aria-label="管理员操作日志"><thead><tr><th>操作人</th><th>动作</th><th>目标</th><th>时间</th><th><span class="sr-only">操作</span></th></tr></thead><tbody><tr v-for="record in rows" :key="record.id"><td><strong>{{ record.actorType === 'system' ? '系统自动任务' : record.actor }}</strong><small>{{ record.actorAccount || (record.actorType === 'system' ? '系统' : '账号') }}</small></td><td><strong>{{ presentAuditAction(record.action).label }}</strong><small>{{ presentAuditAction(record.action).module }}</small></td><td><strong>{{ presentAuditTarget(record).summary }}</strong><small>{{ presentAuditTarget(record).typeLabel }}</small></td><td>{{ formatAuditOccurredAt(record.occurredAt) }}</td><td><button type="button" @click="realList?.select(record)">变更前 / 变更后</button></td></tr></tbody></table></div>
    ...
    <Teleport to="body"><div v-if="selected" class="admin-drawer-backdrop" @click.self="realList?.clearSelection()"><aside class="admin-candidate-drawer" aria-label="日志详情"><header class="admin-drawer__header"><div><span>AUDIT DETAIL</span><h2>{{ presentAuditAction(selected.action).label }}</h2><p>{{ formatAuditOccurredAt(selected.occurredAt) }} · {{ selected.actor }}</p></div><button type="button" aria-label="关闭日志详情" @click="realList?.clearSelection()">×</button></header><div class="admin-drawer__body"><section><header><span>01</span><h3>变更前 / 变更后</h3></header><div class="admin-audit-diff"><article><span>BEFORE</span><p class="admin-audit-projection">{{ formatAuditProjection(selected.before) }}</p></article><article><span>AFTER</span><p class="admin-audit-projection">{{ formatAuditProjection(selected.after) }}</p></article></div></section><section><header><span>02</span><h3>服务端安全字段</h3></header><dl class="admin-detail-grid"><div><dt>目标</dt><dd><strong>{{ presentAuditTarget(selected).summary }}</strong><small>{{ presentAuditTarget(selected).typeLabel }}</small></dd></div><div v-if="selected.reason"><dt>原因</dt><dd>{{ selected.reason }}</dd></div></dl><details class="admin-inline-note"><summary>技术追踪字段</summary><p>{{ selected.action }} · {{ selected.targetType }} / {{ selected.targetId }}</p></details></section></div><footer class="admin-drawer__footer"><button type="button" class="button" @click="realList?.clearSelection()">关闭</button></footer></aside></div></Teleport>
  </div>
</template>
