...
const canReset = computed(() => Boolean(selected.value && identityMatched.value && organizationMatched.value && contactedInPerson.value && reason.value.trim().length >= 10));
...
function resetChecks(request: PasswordRecoveryRequest | null) { identityMatched.value = false; organizationMatched.value = false; contactedInPerson.value = false; reason.value = ""; rejectReason.value = ""; confirmReset.value = false; actionError.value = ""; if (!request) return; }
...
async function applyReject() {
  if (!selected.value || rejectReason.value.trim().length < 10) return;
  actionError.value = "";
