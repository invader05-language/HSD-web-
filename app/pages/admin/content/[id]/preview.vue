...
<template><div class="admin-recruitment-page admin-section-page"><AdminContentRealModeUnavailable v-if="!useMockApi" page="官网内容预览" /><template v-else><AdminPageHeading eyebrow="Preview" :title="record?.title ?? '内容不存在'" description="这是工作版本预览，不代表当前官网公开版本。"><template #actions><NuxtLink v-if="record" class="button button--ghost" :to="`/admin/content/${record.id}`">返回编辑</NuxtLink></template></AdminPageHeading><article v-if="record" class="admin-content-preview"><header><span>{{ PORTAL_CONTENT_KIND_LABELS[record.kind] }}</span><AdminStatusPill :status="PORTAL_CONTENT_STATUS_LABELS[record.status]" /><h2>{{ record.title }}</h2><p>{{ record.summary }}</p></header><div v-for="(block, index) in record.blocks" :key="index"><h3 v-if="block.type === 'heading'">{{ block.text }}</h3><p v-else-if="block.type === 'paragraph'">{{ block.text }}</p><figure v-else><ContentMediaView v-if="block.media" :item="block.media" preview="full" :controls="false" /><div v-else>媒体素材：{{ block.assetId || '未选择' }}</div><figcaption>{{ block.alt || '未填写替代文本' }}</figcaption></figure></div><NuxtLink :to="record.target.value">查看站内目标</NuxtLink></article><div v-else class="admin-empty"><strong>找不到内容</strong><p>请返回列表重新选择记录。</p></div></template></div></template>
