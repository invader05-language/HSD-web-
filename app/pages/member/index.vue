  ...
  <div v-if="profile" class="member-space">
    ...
    <main id="overview">
      ...
      <div class="member-dashboard">
        ...
        <section id="activities"><span>活动与比赛</span><h2>{{ isMockApi ? "2 个待参加事项" : "数据尚未接入" }}</h2><strong>{{ isMockApi ? "查看日程" : "暂不可用" }}</strong><p>{{ isMockApi ? "可在这里取消个人报名。" : "不会使用本地示例代替真实个人数据。" }}</p></section>
      </div>
      <section class="profile-panel"><div><h2>个人资料与头像</h2><p v-if="!isMockApi">资料从服务器读取并持久化；已有头像使用 API 资源，新头像管理尚未接入。</p><p v-else-if="profile.identity === '预备成员'">资料保存后只同步个人中心与招新记录；录取转正前不会进入官网正式成员目录。</p><p v-else>资料保存后会同步到当前前端会话中的成员展示页面；上传头像后自动公开，没有头像时使用白底 HSD 默认图。</p></div><NuxtLink class="button" to="/member/profile">编辑个人资料</NuxtLink></section>
    </main>
  </div>
