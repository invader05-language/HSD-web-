      ...
      applicationStore?.submitApplication(
        {
          ...
          grade: normalizeMemberGrade(profileDraft.grade),
            ...
            <form class="recruitment-application-card" novalidate @submit.prevent="submitApplication">
              <section v-show="step === 1" aria-labelledby="registration-profile-heading">
                ...
                <div class="registration-fields registration-fields--choices">
                  ...
                  <label data-field="studentId"><span>学号</span><input v-model="profileDraft.studentId" inputmode="numeric" maxlength="14" :aria-invalid="Boolean(errors.studentId)" :aria-describedby="errors.studentId ? 'student-id-error' : undefined"><small id="student-id-error" class="form-error registration-field-error" :class="{ 'is-empty': !errors.studentId }" :aria-hidden="!errors.studentId" aria-live="polite">{{ errors.studentId || " " }}</small></label>
                  <label data-field="grade"><span>年级</span><select v-model="profileDraft.grade" :aria-invalid="Boolean(errors.grade)" :aria-describedby="errors.grade ? 'grade-error' : undefined"><option value="">请选择年级</option><option v-for="grade in grades" :key="grade" :value="grade">{{ grade }}</option></select><small id="grade-error" class="form-error registration-field-error" :class="{ 'is-empty': !errors.grade }" :aria-hidden="!errors.grade" aria-live="polite">{{ errors.grade || " " }}</small></label>
                  <label data-field="className"><span>班级</span><input v-model="profileDraft.className" maxlength="30" :aria-invalid="Boolean(errors.className)" :aria-describedby="errors.className ? 'class-name-error' : undefined"><small id="class-name-error" class="form-error registration-field-error" :class="{ 'is-empty': !errors.className }" :aria-hidden="!errors.className" aria-live="polite">{{ errors.className || " " }}</small></label>
                  <label data-field="bio" class="registration-fields__wide"><span>个人简介（选填）</span><textarea v-model="profileDraft.bio" rows="5" maxlength="180" :aria-invalid="Boolean(errors.bio)" :aria-describedby="errors.bio ? 'bio-error' : undefined"></textarea><small>最多 180 个字符，可介绍你的实践重点或想持续发展的方向。</small><small id="bio-error" class="form-error registration-field-error" :class="{ 'is-empty': !errors.bio }" :aria-hidden="!errors.bio" aria-live="polite">{{ errors.bio || " " }}</small></label>
                  <label data-field="contact" class="registration-fields__wide"><span>联系方式</span><input v-model="applicationDraft.contact" maxlength="50" autocomplete="tel" :aria-invalid="Boolean(errors.contact)" :aria-describedby="errors.contact ? 'contact-error contact-help' : 'contact-help'"><small id="contact-help">仅用于招新联系，不展示在公开成员资料中。</small><small id="contact-error" class="form-error registration-field-error" :class="{ 'is-empty': !errors.contact }" :aria-hidden="!errors.contact" aria-live="polite">{{ errors.contact || " " }}</small></label>
                </div>
