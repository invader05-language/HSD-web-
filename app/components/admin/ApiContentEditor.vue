...
type EditableBlock =
  ...
  | { type: "image"; attachmentId: string; alt: string; caption?: string; url?: string; thumbnailUrl?: string };
...
function blockFromRecord(block: AdminContentDetailBlock): EditableBlock {
  ...
  return { type: "image", attachmentId: block.attachmentId, alt: block.alt, ...(block.caption ? { caption: block.caption } : {}), ...(block.url ? { url: block.url } : {}), ...(block.thumbnailUrl ? { thumbnailUrl: block.thumbnailUrl } : {}) };
}
...
async function save() {
  ...
  try {
    ...
    const result = props.record
      ...
      : await gateway!.content.create({ centerId: centerId.value.trim(), kind: kind.value, ...payload } as CreateContentDto);
    notice.value = "草稿已保存。";
...
function appendUploadedImages() {
  ...
  for (const item of attachments) blocks.value.push({ type: "image", attachmentId: item.id, alt: item.alt ?? "", ...(item.caption ? { caption: item.caption } : {}), ...(item.url ? { url: item.url } : {}), ...(item.thumbnailUrl ? { thumbnailUrl: item.thumbnailUrl } : {}) });
  mediaItems.value = [];
}
...
async function command(action: "submit" | "return" | "approve" | "publish" | "publishDirect" | "offline") {
  ...
  if (["publish", "publishDirect", "offline"].includes(action) && !window.confirm(action === "offline" ? "确认下架此内容？" : "确认发布此内容？")) return;
  pending.value = true;
  error.value = "";
  try {
    ...
    const result = action === "submit" ? await gateway.content.submitReview(props.record.id, { expectedVersion: version })
      ...
      : action === "approve" ? await gateway.content.approvePublication(props.record.id, { expectedVersion: version })
        : action === "publish" ? await gateway.content.publish(props.record.id, { expectedVersion: version, confirmed: true })
          : action === "publishDirect" ? await gateway.content.publishDirect(props.record.id, { expectedVersion: version, confirmed: true })
            : await gateway.content.offline(props.record.id, { expectedVersion: version, reason: reason.value.trim() });
    notice.value = action === "offline" ? "内容已下架。" : ["publish", "publishDirect"].includes(action) ? "内容已发布到官网。" : "内容状态已更新。";
    adminToast.success(notice.value);
    emit("saved", result.id);
  } catch (cause) { fail(cause); }
  finally { pending.value = false; }
        ...
        <section>
          ...
          <div class="admin-editor-grid">
            ...
            <template v-if="kind === 'flash'">
              <label class="is-wide">摘要（可选）<textarea v-model="summary" :disabled="isReadOnly" rows="2" placeholder="用于首页快讯和详情页的简短说明"></textarea></label>
              <label>标签<input v-model="tag" :disabled="isReadOnly" placeholder="例如：活动通知"></label>
              <label>关联页面<select v-model="internalTarget" :disabled="isReadOnly"><option value="/updates">新闻动态</option><option value="/activities">动态与活动</option><option value="/gallery">媒体画廊</option><option value="/projects">项目成果</option></select></label>
        ...
        <section>
          ...
          <div class="admin-content-blocks">
            <article v-for="(block, index) in blocks" :key="index">
              ...
              <div v-else class="admin-inline-note"><img v-if="block.url" :src="resolveApiMediaUrl(block.url, apiBase)" alt="" class="admin-content-editor__inline-image"><span>正文图片附件：{{ block.attachmentId || "尚未上传" }}</span><input v-model="block.alt" :disabled="isReadOnly" placeholder="图片替代文本"><input v-if="block.caption !== undefined" v-model="block.caption" :disabled="isReadOnly" placeholder="图片说明（可选）"></div>
