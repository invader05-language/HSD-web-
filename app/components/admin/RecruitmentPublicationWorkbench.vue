...
<template>
  <div class="admin-recruitment-page admin-section-page">
    ...
    <p v-if="feedback" class="admin-save-message" role="status">{{ feedback }}</p><p v-if="error" class="admin-save-message" role="alert">{{ error }}</p><p v-else-if="apiError" class="admin-save-message" role="alert">{{ getRecruitmentAssessmentMessage(apiError, "批次结果加载失败，请刷新后重试。") }}</p>
