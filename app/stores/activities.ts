  ...
  actions: {
    ...
    getPublicActivities(): PublishedActivity[] {
      return this.activities
        .filter((activity) => activity.publishedState === "published" && activity.publishedSnapshot)
        .map((activity) => {
          const registrationOpen = isActivityRegistrationOpen(activity);
          return { ...clone(activity.publishedSnapshot!), registrationOpen, ...(activity.registrationOverride ? { registrationOverride: true } : {}) } as PublishedActivity & { registrationOpen: boolean };
        });
    },
    getPublicBySlug(slug: string) {
      const activity = this.activities.find((item) => item.slug === slug && item.publishedState === "published" && item.publishedSnapshot);
      if (!activity?.publishedSnapshot) return undefined;
      const registrationOpen = isActivityRegistrationOpen(activity);
      return { ...clone(activity.publishedSnapshot), registrationOpen, ...(activity.registrationOverride ? { registrationOverride: true } : {}) } as PublishedActivity & { registrationOpen: boolean };
    },
