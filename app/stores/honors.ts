...
type HonorsGateway = { listAdmin(): Promise<{ items: AdminHonorResponseDto[] }>; approve(id: string, expectedVersion: number): Promise<AdminHonorResponseDto>; softDelete(id: string, expectedVersion: number): Promise<unknown> }
...
export const useHonorsStore = defineStore('honors', {
  ...
  actions: {
    ...
    async softDelete(gateway: HonorsGateway, id: string, publicId: string, expectedVersion: number, confirmed: boolean) { if (!confirmed) return false; this.apiLoading = true; this.apiError = null; try { await gateway.softDelete(publicId, expectedVersion); this.items = this.items.filter((item) => item.id !== id); return true } catch (error) { this.apiError = errorFrom(error); return false } finally { this.apiLoading = false } },
  },
})
