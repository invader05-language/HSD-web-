...
export function createAdminResourceListController(gateway: AdminResourceListGateway, initialQuery: Partial<AdminResourceListQuery> = {}) {
  ...
  async function load() {
    ...
    try { const response = await gateway.list(toQueryString(query.value)); if (generation !== requestGeneration) return; query.value = { ...query.value, page: response.page, pageSize: response.pageSize }; records.value = response.items.map(mapAdminResourceSummary); total.value = response.total; status.value = response.items.length ? "success" : "empty"; }
