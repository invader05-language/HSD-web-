...
export class ContentApiError extends Error { constructor(readonly status: number, readonly code: string, message: string, readonly requestId?: string, readonly fieldErrors?: Record<string, string>) { super(message); this.name = "ContentApiError" } }
...
export function createApiContentGateway(options: ApiContentGatewayOptions) {
  ...
  const transport: ApiTransport = async (request: ApiRequest) => {
    ...
    if (!response.ok) throw new ContentApiError(response.status, isError(payload) ? payload.code : "CONTENT_API_REQUEST_FAILED", isError(payload) ? payload.message : "Content API request failed", isError(payload) ? payload.requestId : undefined, isError(payload) ? payload.fieldErrors : undefined);
    return payload;
  };
  ...
  async function readPublicCollection(path: string): Promise<{ items: Array<Record<string, unknown>>; page?: number; pageSize?: number; total?: number }> {
    ...
    const payload: unknown = await response.json();
    if (!response.ok) throw new ContentApiError(response.status, isError(payload) ? payload.code : "PUBLIC_CONTENT_REQUEST_FAILED", isError(payload) ? payload.message : "Public content request failed", isError(payload) ? payload.requestId : undefined, isError(payload) ? payload.fieldErrors : undefined);
